...
router.post("/create", async (req, res) => {
  let { title, state, description, city, category, image, address } = req.body;
  try {
    let errors = [];
    // Validation checks for other fields...
    if (!address || address.length < 5 || address.length > 30)
      errors.push(
        "Address should be at least 5 characters long and max 30 characters long; "
      );
...
router.patch("/edit/:id", isAuth, async (req, res) => {
  //TODO: Rewrite this
  let { title, state, description, city, category, image } = req.body;
  try {
    ...
    if (/^[A-Za-z]+$/.test(state) == false)
      errors.push("State should contains only english letters; ");
    if (req.body.image) {
